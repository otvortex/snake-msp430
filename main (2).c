...
void createNewApple();			//creates new random apple
...
volatile unsigned int state = 0; 		//if 0, game paused; if 1 game running;
...
int main(){
	...
	//initialize game to state = 1, means running
	state = 1;
...
interrupt void WDT_interval_handler(){
  ...
  //check for button pressed for each button, debounced, and generates pseudo-random X and Y based on timing of button press
  if(P1IN & BIT1)	{
	  randomX = randomInt%8;
	  button_1_pressed = 1;
  }
...
//volatile unsigned int random=0;
volatile unsigned int randomTemp;

void createNewApple() {
	unsigned int i, j;
	int random = 0;
	random = (randomX+1)*(randomY+1);
	for( i = 0; i < 8; i++ ) {
		for( j = 0; j < 8; j++) {
			if(!(arr[i][j])) {
				if(!(random--))	{
